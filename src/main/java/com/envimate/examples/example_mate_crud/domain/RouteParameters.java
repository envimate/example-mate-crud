...
public final class RouteParameters {
    ...
    public static RouteParameters routeParameters(final RouteParameter... parameters) {
        return new RouteParameters(Optional.ofNullable(parameters).map(Arrays::asList).orElse(List.of()));
    }

    public static RouteParameters empty() {
        return new RouteParameters(List.of());
    }

    public Map<String, String> asMap() {
        return parameters.stream().collect(Collectors.toMap(o -> o.key.internalValue(), o -> o.value.internalValue()));
    }
}
